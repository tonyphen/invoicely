  ...
  <div class="page-invoices">
    <nav class="breadcrumb" aria-label="breadcrumbs">
      <ul>
        ...
        <li class="is-active">
          <router-link :to="{name: 'Invoice', params: {id: invoice.id}}" aria-current="true">{{
...
export default {
  ...
  methods: {
    getInvoice() {
      ...
      axios
          .get(`/api/v1/invoices/${invoiceID}`).then(response => {
    ...
    getPdf() {
      const invoiceID = this.$route.params.id
      axios.get(`/api/v1/invoices/${invoiceID}/generate_pdf/`, {responseType: 'blob',})
          .then(res => {
    ...
    async createCreditNote(){
      ...
      let items = this.invoice.items
      delete this.invoice['items']
      await axios.patch(`/api/v1/invoices/${this.invoice.id}`, this.invoice).then(response =>{
        toast({
          message: 'The changes was saved',
          type: 'is-success',
          dismissible: true,
      ...
      await axios.post('/api/v1/invoices/', creditNote).then(response=>{
        toast({
    ...
    sendReminder(){
      axios.get(`/api/v1/invoices/${this.invoice.id}/send_reminder/`).then(response=>{
        toast({
